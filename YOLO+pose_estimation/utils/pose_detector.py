...
from utils.entity import params, JointType
from utils.media_reader import ImageReader, get_filename_without_extension
        ...
        for i, (limb, color) in enumerate(zip(params['limbs_point'], limb_colors)):
            if i != 9 and i != 13:  # don't show ear-shoulder connection
                ...
                if np.all(pose[limb_ind][:, 2] != 0):
                    ...
                    distance = math.sqrt(((joint1[0] - joint2[0]) * (joint1[0] - joint2[0])) + (
                                (joint1[1] - joint2[1]) * (joint1[1] - joint2[1])))
        ...
        if all_distance >= 300:
            for i, (limb, color) in enumerate(zip(params['limbs_point'], limb_colors)):
                if i != 9 and i != 13:  # don't show ear-shoulder connection
                    ...
                    if np.all(pose[limb_ind][:, 2] != 0):
                        joint1, joint2 = pose[limb_ind][:, :2]
                        cv2.line(canvas, tuple(joint1), tuple(joint2), color, 3)
            # 관절
            points = []
